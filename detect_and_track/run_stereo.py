#!/usr/bin/env python3
"""
YOLO-3D with Fast-FoundationStereo depth estimation for KITTI stereo dataset.

Input: KITTI stereo image pairs (left/right directories)
Output: 3D bounding box visualization with metric depth
"""
# Must be set BEFORE any torch import to prevent torch.compile failures
# when Triton is not installed (Fast-FoundationStereo uses @torch.compile)
import os
...
# Determine Fast-FoundationStereo root
_code_dir = os.path.dirname(os.path.realpath(__file__))
_ffs_root = os.environ.get('FFS_ROOT', os.path.join(_code_dir, '..'))
_ffs_root = os.path.abspath(_ffs_root)
...
def main():
    ...
    parser.add_argument('--ckpt_dir', type=str,
                        default=r'C:\Users\GGG\Downloads\20-30-48-20260705T074058Z-3-001\20-30-48\model_best_bp2_serialize.pth',
                        help='Path to Fast-FoundationStereo serialized model (.pth). '
                             'e.g. weights/23-36-37/model_best_bp2_serialize.pth. '
                             'If not set, looks for FFS_ROOT/weights/23-36-37/model_best_bp2_serialize.pth')
    parser.add_argument('--output_path', type=str, default='output_faststereo.mp4',
    ...
    parser.add_argument('--valid_iters', type=int, default=8,
                        help='Number of GRU iterations (default 8 for real-time, reduce to 4 for faster)')
    ...
    # Default ckpt path if not provided
    if args.ckpt_dir is None:
        ffs_root = os.environ.get('FFS_ROOT', os.path.join(code_dir, '..'))
        args.ckpt_dir = os.path.join(ffs_root, 'weights', '23-36-37', 'model_best_bp2_serialize.pth')
        print(f"Using default Fast-FoundationStereo weights: {args.ckpt_dir}")

    # Initialize Fast-FoundationStereo depth estimator
    print("Initializing Fast-FoundationStereo depth estimator...")
